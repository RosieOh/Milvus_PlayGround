...
기획서가 정한 성공 기준 8개 중 **6개 달성, 2개 미달**. 미달도 그대로 싣는다.

| 기준 | 목표 | 실측 | |
|---|---|---|:-:|
| 재색인 중 다운타임 | 0 s | **0** | ✅ |
| 쓰기 유실 | 0 건 | **0 / 45** | ✅ |
| 부하 중 오류 | 0 건 | **0 / 45,010** | ✅ |
| alias 스왑 소요 | ≤ 5 s | **281 ms** | ✅ |
| 롤백 소요 | ≤ 10 s | **148 ms** | ✅ |
| 품질 게이트 | 회귀 시 중단 | **통과** nDCG@10 0.8208 → 0.8455 | ✅ |
| 재색인 중 p99 증가 | ≤ +15 % | 전환 1초 창에서 **+94 %** (읽기만) · **+436 %** (쓰기 동시) | ⚠️ |
| 전량 재색인 소요 | ≤ 30 min | 1.4 M 외삽 **약 3.9 h** (fp16) · 이 장비로는 도달 불가 | ⚠️ |

dev 티어 50 K · `multilingual-e5-small`(384d) → `e5-base`(768d) · 워커 8 · Zipf 질의 ·
읽기 부하 30 s 중간에 스왑 후 롤백, 쓰기 8/s 동시 수행.

**절대 수치가 아니라 동일 조건 상대 비교로만 읽을 것** — 단일 노트북 벤치다.

**재색인 30분 목표는 레버를 전부 재고 도달 불가로 결론냈다.** 병목은 MPS 임베딩이고
(적재 490 rows/s 대 임베딩 108 rows/s), 배치 크기는 64 이하에서 평평한 천장이며,
fp16 은 품질 게이트를 통과하며 +17.8 % 를 준다. 가장 낙관적인 예측(fp16 + 병렬화 3.0 h)도
**5.9 배 모자란다.** 목표를 바꿔 "달성"으로 만들지 않았다 — D-032.
p99 의 두 값이 다른 이유는 아래 「재색인」 절에 있다. 쓰기 쪽 악화는 **Milvus 가 아니라
이 하네스의 한계**다 — 임베딩 인코더가 스레드 안전하지 않아 읽기·쓰기가 락을 공유한다.
...
무중단 스왑은 **세 번 실패한 뒤에** 됐다. 그 과정이 [DECISIONS.md](DECISIONS.md) 29건에 있다.
...
**트래픽이 실제로 옮겨갔다는 증거**: v2 가 10,520 건을 처리했다(t=13~23s).
...
전환은 **1초 창**(재해석 주기)에 끝나고 그 창의 p99 만 **+94 %** 튄다.
v2 정상 상태의 p99 +19 % · 처리량 −24 % 는 스왑이 아니라 **768 차원의 비용**이다.

#### 체제 B — 쓰기 8/s 동시 (하네스 한계가 드러나는 수치)

| 구간 | 대상 | p99 | 처리량 |
|---|---|---:|---:|
| t=0~9 | v1 | ~10.9 ms | ~1,774/s |
| **t=13** | **v1:318 / v2:876** | **58.37 ms** | 1,194 |
| t=14~22 | v2 | ~33.8 ms | ~1,113/s |
| t=25~29 | v1 (롤백 후) | ~12.9 ms | ~1,956/s |

**이 악화는 Milvus 가 아니라 이 하네스의 비용이다.** 임베딩 인코더가 스레드 안전하지
않아(MPS) 쓰기 경로와 읽기 경로가 락을 공유한다. 실서비스는 임베딩을 별도 프로세스로
분리하므로 이 값이 그대로 나타나지 않는다 — D-029 의 재검토 조건에 적어뒀다.

**세 가지 비용을 섞지 말 것**: ① 전환 자체(1초 창) ② 768 차원(정상 상태)
③ 이 하네스의 인코더 락(체제 B). 셋을 합쳐서 "재색인이 느리다"고 쓰면 틀린다.
